...
<div>
  <ol class="breadcrumb">
    ...
    <li><a href="${pageContext.request.contextPath}/app/normalizzatore/cercavenditore/init">Cerca Venditore</a></li>
    <li><a href="${pageContext.request.contextPath}/app/venditore/visualizza/${venditore.id}">${venditore.nome }</a></li>
    ...
    <div class="panel-body">
      ...
      <div id="tabbContent" class="tab-content">
        <div id="outbound" class="tab-pane fade active in" style="margin-top: 10px;">
          <a class="btn btn-primary" style="margin-bottom: 10px;" href="${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/${venditore.id}/OUTBOUND/nuovo">Nuovo Template</a>
        ...
        <div id="inbound" class="tab-pane fade" style="margin-top: 10px;">
          ...
          <a class="btn btn-primary" style="margin-bottom: 10px;" href="${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/${venditore.id}/INBOUND/nuovo">Nuovo Template</a>
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col-lg-6" align="center" style="border-right: #cccccc 1px solid">
            ...
            <a class="btn btn-info" id="downloadTemplateFile" href="${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/{id}/dowload"><span class="fa fa-download"></span> Download</a>
          </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <dl class="dl-horizontal">
          ...
          <dt>Distributore</dt>
          <dd>
            <select class="form-control input-sm" id="listVenditori">
              <c:forEach var="itemVenditore" items="${listVenditori}">
                <c:choose>
                  <c:when test="${venditore.id != itemVenditore.id }">
                    <option value="${itemVenditore.id}">${itemVenditore.nome}</option>
                  </c:when>
                </c:choose>
              </c:forEach>
            </select>
...
function clonaTemplate(id, idDistributore, codiceServizio, codiceFlusso, utility){
    ...
    $("#btnCloneConfirm").on('click', function(e){
      ...
      $.ajax({
          async: false,
          url: "${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/clona",
          data: {
              idTemplate: id,
              idVenditore: $("#listVenditori").val(),
              ...
              if(data.codErrore){
                ...
                $('<p/>').text("Template clonato con successo.").append("<a href=\"${pageContext.request.contextPath}/app/templatevenditore/" + data.idTemplateNew + "/visualizza\" class=\"alert-link\">Vai al template</a>").prepend('<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>').addClass("alert alert-success").appendTo('#cloneResult');
              }
          });
   });
}
...
function modificaFile(id){
  ...
  $('#progress').css('display', 'none');
	$("#idTemplate").val(id);
	$("#downloadTemplateFile").attr('href', '${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/' + id + '/download');
...
$(function () {
    ...
    var url = "${pageContext.request.contextPath}/app/json/normalizzatore/templatevenditore/upload";
  ...
  $('#tblTemplateInstanceOutbound').dataTable({
      ...
      sAjaxSource: "${pageContext.request.contextPath}/app/json/normalizzatore/templatevenditore/${venditore.id}/elenco/OUTBOUND",
      bSort: false,
      aoColumns: [
               ...
               {mData: "id", sClass: "middle", mRender: function ( data, type, full ) {
            	   return '<a title="Clona Template" href="#" onclick="clonaTemplate($(this).data(\'id\'), $(this).data(\'idVenditore\'), $(this).data(\'codiceServizio\'), $(this).data(\'codiceFlusso\'), $(this).data(\'utility\'))" data-id="'+data+'" data-id-venditore="${venditore.id}" data-codice-servizio="'+full.codiceServizio.code+'" data-codice-flusso="'+full.codFlusso.id+'" data-utility="'+full.utility+'" class="pointer"><span class="fa fa-link"></span></a>';
               }},
               {mData: "id", sClass: "middle", mRender: function ( data, type, full ) {
                   return '<a title="Visualizza Template" href="${pageContext.request.contextPath}/app/normalizzatore/templatevenditore/' + data + '/visualizza" data-id="'+data+'" class="pointer"><span class="fa fa-edit"></span></a>';
               }},
             ]
    });
  $('#tblTemplateInstanceInbound').dataTable({
      sDom: "<'row'<'col-xs-6'l><'col-xs-6'f>r>t<'row'<'col-xs-6'i><'col-xs-6'p>>",
      sAjaxSource: "${pageContext.request.contextPath}/app/json/normalizzatore/templatevenditore/${venditore.id}/elenco/INBOUND",
      bSort: false,
      aoColumns: [
               {mData: "codiceServizio.code", sClass: "middle", mRender: function ( data, type, full ) {
