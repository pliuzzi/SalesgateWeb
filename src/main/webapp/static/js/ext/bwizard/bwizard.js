	...
	_refreshStep: function () {
		...
		if (this.lis) {
			...
			if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
				if (this.lis) {
					this.lis.eq(o.activeIndex).removeClass('prev').addClass('active').attr('aria-selected', true).find('.label').addClass('badge-inverse');
				}
			}
			for(i=0; i < o.activeIndex; i++){
				this.lis.eq(i).addClass('prev');
			}
		}
