...
public class TemplateInstanceValidator implements Validator {
  ...
  @Autowired
  Validator cloneTemplateInstanceValidator;
  ...
  public void validate(Object target, Errors errors) {
    ...
    /*
     * int n = templateService.verifiyEventCode(ti); if(n > 0){ errors.rejectValue("codiceServizio.code", "required",
     * "Template esistente per questo codice servizio"); errors.rejectValue("codFlusso.id", "required",
     * "Template esistente per questo codice flusso"); }
     */
    if (StringUtils.isEmpty(ti.getId()))
      ValidationUtils.invokeValidator(cloneTemplateInstanceValidator, target, errors);
