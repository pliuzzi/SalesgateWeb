...
public class UtilityDAOImpl extends AbstractDAO implements UtilityDAO {
  ...
  public List<FlussiSalvabili> getAllFlussiSalvabili(LavoriGas pratica) {
    ...
    return jdbcTemplateSdm.query(sql, new FlussiSalvabiliJdbcHandler().getRowMapper(), pratica.getCodServizio(), pratica.getCodFlusso(), pratica.getStato(), pratica.getUtility());
  }
  ...
  public List<Params> getParams(String category) {
    List<Params> list = jdbcTemplateSdm.query("SELECT ID, NAME, VALUE, CATEGORY FROM PARAMS where category = ?", new ParamsJdbcHandler().getRowMapper(), category);
